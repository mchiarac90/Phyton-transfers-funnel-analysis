...
### 1. Import the excel file is in the root directory
<pre><code> df = pd.read_excel('funnel_data_sample.xlsx') </code></pre>

### 2. Count each unique event name to build the funnel stages
<pre><code> funnel_counts = df['event_name'].value_counts() </code></pre>

### 3. Extract specific counts for the core funnel stages
...
### 5. Create funnel table
#### 5.1 Define the order of stages and construct a specific DataFrame for the chart
...
#### 5.2 Normalize calculations: calculate the percentage of total based on the 'Transfer Created' volume
...
#### 5.6 Display the chart
<pre><code> fig.show() </code></pre>

### 6. Analyze correlations
#### 6.1 Create binary flags (1 or 0) for transfer state to allow easy summation during grouping
...
#### 6.2 Group by the dimensions that impact user behavior: Region, Experience, and Platform
